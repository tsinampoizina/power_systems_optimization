...
def clean_metadata(filename):
    
    df = pd.read_csv(filename, skiprows=16, nrows=1,
...
def _download_ftp_file(ftp_handle, name, dest, overwrite):
    ...
    if not os.path.exists(dest) or overwrite is True:
        try:
            ...
            clean_metadata(local_path_of_csv)
...
if __name__ == "__main__":
    ...
    # local_dir = "/Volumes/WD-DATA-100/power_optimisation_system/metadata/solar_metadata/"
    # for wind speed
    remote_dir = "/badc/ukmo-midas-open/data/uk-mean-wind-obs/dataset-version-202107/"
    local_dir = "/Volumes/WD-DATA-100/power_optimisation_system/metadata/wind_metadata/"
    # for all years
    pattern = ".*midas-open_uk.*_capability.csv"
    # for some years
